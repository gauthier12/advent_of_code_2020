...
fn check_ruleb(rule: &Rule) -> bool {
    if rule.word.chars().count() < rule.max_repetition as usize {
        false
    } else {
        (rule
            .word
            .chars()
            .nth((rule.min_repetition as usize) - 1)
            .unwrap()
            == rule.letter)
            ^ (rule
                .word
                .chars()
                .nth((rule.max_repetition as usize) - 1)
                .unwrap()
                == rule.letter)
    }
}
...
fn main() {
    ...
    if let Ok(contents) = fs::read_to_string(filename) {
        ...
        for cur_rule in rule_list.iter() {
            tot_a += check_ruleb(&cur_rule) as u32;
        }
        let duration_a = start_a.elapsed();
        println!("Methode B");
        let start_b = Instant::now();
        let tot_b: u32 = rule_list.iter().map(|cr| (check_ruleb(&cr) as u32)).sum();
        let duration_b = start_b.elapsed();

        println!("Methode C");
        let start_c = Instant::now();
        let tot_c: u32 = rule_list
            .par_iter()
            .map(|cr| (check_ruleb(&cr) as u32))
            .sum();
